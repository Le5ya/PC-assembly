const time = () => {
	...
	const numWord = (value, word) => {
		value = Math.abs(value) % 100
		const lastNum = value % 10
		if (value > 10 & value < 20) return word[2]
		if (lastNum > 1 && lastNum < 5) return word[1]
		if (lastNum === 1) return word[0]
		return word[2]
	}
	...
	const updateTimer = () => {
		...
		daysBlock.nextElementSibling.textContent = numWord(days, ['день', 'дней', 'день'])
